...
abstract contract BaseSinglePair is BaseTest {
        ...
        )
    {
        ...
        uint256 amountIn = _flt.router().getAmountsIn(_totalCollateral, path)[
            0
        ];
    ...
    function testDeployRevertIfDeployedTwice() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IFLT.Deployed.selector));
        _flt.deploy(
    ...
    function getAmountInViaDebt(FLTSinglePair _token, uint256 _shares)
        internal
        view
    ...
    function getAmountInViaCollateral(FLTSinglePair _token, uint256 _shares)
        internal
        view
        returns (uint256 _amountIn)
    {
    ...
    function getAmountIn(
        ...
        uint256 _shares,
        address _tokenIn
    ) internal view override returns (uint256 _amountIn) {
        FLTSinglePair _flt = FLTSinglePair(_token);
    ...
    function getAmountOutViaDebt(FLTSinglePair _token, uint256 _shares)
        internal
        view
    ...
    function getAmountOutViaCollateral(FLTSinglePair _token, uint256 _shares)
        internal
        view
        returns (uint256 _amountOut)
    {
    ...
    function testSetParamsRevertIfMinMaxLeverageRatioInvalid() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IFLT.InvalidLeverageRatio.selector)
        );
    ...
    function testSetParamsRevertIfMaxDriftTooLow() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IFLT.InvalidMaxDrift.selector));
    ...
    function testSetParamsRevertIfMaxIncentiveTooLow() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IFLT.InvalidMaxIncentive.selector)
        );
