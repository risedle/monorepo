...
contract FLTRebalancer {
     ...
     */
    function getRebalances(
        address[] memory _flts,
        uint256[] memory _minProfits
    ) external view returns (bytes[] memory _calls) {
        ...
        returns (uint256 _amountIn, uint256 _amountOut)
    {
        ...
        _amountIn = oracle.totalValue(c, d, estAmountOut);
        ...
        returns (uint256 _amountIn, uint256 _amountOut)
    {
        ...
        uint256 maxIncentive = incentive.mulWadDown(maxAmountOut);
        uint256 estAmountOut = maxAmountOut - maxIncentive;
        _amountIn = oracle.totalValue(d, c, estAmountOut);
        _amountOut = estAmountOut + incentive.mulWadDown(estAmountOut);
    }
        ...
        returns (bytes memory _calldata)
    {
        ...
        (uint256 amountIn, uint256 amountOut) = getLeverageDownInOut(_flt);
        ...
        returns (bytes memory _calldata)
    {
        ...
        (uint256 amountIn, uint256 amountOut) = getLeverageUpInOut(_flt);
...
interface FLT {
    ...
    function value(uint256 _shares) external view returns (uint256);

    function oracleAdapter() external view returns (Oracle);
